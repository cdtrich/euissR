#' Custom ggplot2::geom_linerange_y()
#'
...
#' @param ... other arguments that can be passed to \code{ggplot2::geom_linerange_y()}
#'
#' @return Pre-styled \code{geom_linerange_y()}
#' @export
#'
#' @examples 
#' require(dplyr)
...
#'   ggplot2::ggplot(ggplot2::aes(x = cyl, y = wt, ymax = wt, group = gear)) + 
#'   geom_linerange_y_euiss() + 
#'   ggplot2::facet_wrap(~gear)
geom_linerange_y_euiss <- function(mapping = NULL, 
                                   data = NULL, 
                                   # y = NULL,
                                   ymin = 0,
                                   shape = 21,
